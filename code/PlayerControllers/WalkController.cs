...
namespace htkgttt
{
	...
	{
		[Net] public float SprintSpeed { get; set; } = 80.0f;
		[Net] public float WalkSpeed { get; set; } = 50.0f;
		[Net] public float DefaultSpeed { get; set; } = 60.0f;
		...
		public virtual float GetWishSpeed()
		{
			...
			if(Pawn is Player ply )
			{
				SpeedMult = ply.GetAnimParameterInt( "b_IK_arm" ) == 2 ? 1f : 2f;
			}

			if ( Input.Down( InputButton.Run ) ) return SprintSpeed * SpeedMult;
			if ( Input.Down( InputButton.Walk ) ) return WalkSpeed * SpeedMult;

			return DefaultSpeed * SpeedMult;
		}
